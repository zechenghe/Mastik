...
taskset 0x20 $quickhpc -c hpc_config_$HPC_COLLECTION -a $SENSITIVE_PROGRAM_L1_PID -i $INTERVAL_US > $OUTPUT_FOLDER/hpc_sensiprog_l1 &
QUICKHPC_L1_PID=$!
taskset 0x40 $quickhpc -c hpc_config_$HPC_COLLECTION -a $SENSITIVE_PROGRAM_L3_PID -i $INTERVAL_US > $OUTPUT_FOLDER/hpc_sensiprog_l3 &
QUICKHPC_L3_PID=$!
